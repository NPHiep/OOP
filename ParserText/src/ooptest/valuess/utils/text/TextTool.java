...
public class TextTool {
    ...
    public TextTool(String content) {
        ...
        while(i < content.length()){
        	...
        	if (Character.isLetterOrDigit(c)){
        		int start = i;
        		boolean drop = false;
        		if (Character.isDigit(c) || (Character.isUpperCase(c) && !startSentence)) drop = true;
        		do{
        			i++;
        			...
        			if (c == '_' || c == '.' ||  Character.isDigit(c)) drop = true;
        		} while (Character.isLetterOrDigit(c) || c == '.' || c == '\'' || c == '-' || c == '_');
        		
        		// if word and with a not letter, drop word
        		if (!Character.isLetter(content.charAt(i-1))) drop = true;
        		
        		if (drop)
        			for (int k = start; k < i; k++) builder.append(' ');
        		else
    ...
    public Word[] toWordArray() {    	 
        ...
        for (int i = 0; i < strList.size(); i++) {        	
        	...
        	if (word.length() < 2) continue;  // throw word which of length is 1 (contains case common words)
        ...
        while (index + word.length() < content.length()) {
        	...
        	for (start = index - 1; start >= 0; start--){
        		...
        		if (c == '|') break;
                    ...
                    if (!inWord) {countWord++; inWord = true;}
                }
            }
        	start ++;
    ...
    private String trimString(String str){
        ...
        for (int index = 0; index < str.length(); index++){
            ...
            if (c == '\n'){
                if (index == 0 || Character.isLetterOrDigit(str.charAt(index - 1))) builder.append(' ');
            } else
                builder.append(c);
        }
        ...
        for (start = 0; start < str.length(); start++){
            if (Character.isLetterOrDigit(str.charAt(start))) break;
        }
        for (end = str.length() - 1; start < end; end--){
            if (Character.isLetterOrDigit(str.charAt(end))) break;
        }
